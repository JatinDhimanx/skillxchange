...
export const HeaderNav: React.FC<HeaderNavProps> = ({ currentTab, onSelectTab }) => {
  ...
  return (
    <>
      <header
        ref={navRef}
        className="sticky top-0 z-50 w-full bg-white/96 backdrop-blur-md border-b border-slate-200 shadow-xs"
      >
        <div className="max-w-[1360px] mx-auto px-2 sm:px-4 h-16 flex items-center justify-between gap-2 sm:gap-3">
          ...
          >
            <span className="font-display font-black text-xl sm:text-2xl text-slate-900 tracking-tight whitespace-nowrap group-hover:opacity-90 transition-opacity">
            ...
            <nav className="hidden lg:flex items-center flex-nowrap shrink-0 gap-0.5 p-1 rounded-full bg-slate-100/80 border border-slate-200/80">
              {mainNavItems.map(item => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full transition-all whitespace-nowrap shrink-0 cursor-pointer ${isActive
              ...
              <div className="relative shrink-0">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full transition-all whitespace-nowrap shrink-0 cursor-pointer ${isLabActive
                  ...
                  <div className="absolute left-0 mt-2 w-72 p-2 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 space-y-1 animate-slide-down">
                    ...
                    {innovationItems.map(lab => (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 p-2.5 rounded-xl text-left transition-all text-xs cursor-pointer ${currentTab === lab.id ? 'bg-slate-100 text-slate-900 font-bold' : 'hover:bg-slate-50 text-slate-700'
                          }`}
                      >
                    ...
                    {notifications.length > 0 && (
                      <div className="flex items-center gap-1 p-0.5 rounded-lg bg-slate-100 text-[11px] font-semibold">
                        <button
                          ...
                          className={`flex-1 py-1 rounded-md text-center transition-all ${notifFilter === 'all' ? 'bg-white text-slate-900 shadow-xs font-bold' : 'text-slate-500 hover:text-slate-800'
                            }`}
                        >
                        ...
                        <button
                          ...
                          className={`flex-1 py-1 rounded-md text-center transition-all ${notifFilter === 'unread' ? 'bg-white text-slate-900 shadow-xs font-bold' : 'text-slate-500 hover:text-slate-800'
                            }`}
                        >
                          Unread ({unreadCount})
                        </button>
                      ...
                      {(() => {
                        ...
                        return list.map(n => (
                          <div
                            ...
                            className={`p-2.5 rounded-xl border text-xs cursor-pointer transition-all ${n.read
                ...
                <div className="grid grid-cols-2 gap-2">
                  {mainNavItems.map(item => (
                    <button
                      key={item.id}
                      onClick={() => handleTabClick(item.id)}
                      className={`flex items-center gap-2 px-3 py-3 rounded-xl text-xs font-semibold transition-all active:scale-95 ${currentTab === item.id ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                        }`}
                    >
                      {item.icon}
                      <span>{item.label}</span>
                ...
                <div className="grid grid-cols-1 gap-1.5">
                  {innovationItems.map(lab => (
                    <button
                      key={lab.id}
                      onClick={() => handleTabClick(lab.id)}
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all active:scale-95 ${currentTab === lab.id ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-700 hover:bg-slate-100'
                        }`}
                    >
